	...
	<div class="app-container">
		<div class="filter-container">
			<el-select 
				...
				class="searchClass">
				<el-option 
					v-for="f in perFormulas" 
					:key="f.id" 
					:label="f.name" 
					:value="f.id">
					<span style="float: left">{{ f.name }}</span>
					<el-tooltip :content="'共'+f.personNum+'人'"
						placement="right">
			...
			<el-select 
				...
				class="searchClass">
				<el-option 
					v-for="p in personParams" 
					:key="p.id" 
					:label="p.name + '_' + p.departmentName" 
					:value="p.id">
		...
		<el-table 
			...
			<el-table-column prop="perCode" label="人员编码">
			...
			<el-table-column prop="perName" label="姓名">
				<template slot-scope="scope">
					<el-tooltip :content="scope.row.posName" 
						placement="right">
		...
		<per-run-detail
			...
			@closePerRunDetail="closePerRunDetail">
		</per-run-detail>
...
<script>
import { getPerRunList, doPerRun, oneKeyRun } from "@/api/bnsForPerRun";
...
import PerRunDetail from "./components/PerRunDetail";
export default {
  name: "DeptRun",
  directives: { waves },
  components: { countTo, PerRunDetail },
  data() {
    return {
      list: [],
      total: 0,
  ...
  methods: {
    ...
    getList() {
      ...
      getPerRunList(query).then(response => {
        self.accountStatus = response.data.accountStatus;
        const runs = response.data.perRuns;
    ...
    getPerFormulaParams() {
      ...
      getFormulas(query).then(response => {
        this.perFormulas = response.data;
      });
    },
    getDepartmentParams() {
    ...
    getDeptRunInfo(data) {
      this.runData.id = data.perRunID;
      this.dialogStatus = data.perName + data.formulaName + "的公式明细";
    ...
    calcPerRun(data) {
      ...
      const perRunVO = {
        perRunID: data.perRunID,
        yearmonth: data.yearmonth
      };
      doPerRun(perRunVO).then(response => {
        ...
        if (resultStatus === 520) {
          ...
          let successMsg = data.perName + data.formulaName + "重新核算完成";
          self.$notify({
            title: "成功",
            message: successMsg,
            type: "success",
