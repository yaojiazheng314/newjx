			...
			<el-form 
				...
				<el-form-item label="目标类型" 
					...
					<el-radio-group 
						...
						<el-radio :disabled="!yearAimFlg" label="year">年度目标</el-radio>
						<el-radio 
							:disabled="yearAimFlg"  
							label="month">月度目标</el-radio>
					</el-radio-group>
				</el-form-item>
				<!-- 目标执行说明 -->
				...
				<el-form-item 
					...
					<!-- 用户可操作的目标 -->
					<el-select 
						v-model="aimConfigForm.parent" 
						filterable clearable 
						placeholder="选择上级目标">
						<el-option 
							v-for="item in usersAimParams" 
							:key="item.id" 
							:label="item.name" 
							:value="item.id">
							<span style="float: left">{{ item.name }}</span>
...
export default {
	...
	name: 'AimConfigEditDialog',
	components: { AimChildWeight },
	data() {
		return {
			showClose: false,
			loading: false, //用于保存
			yearAimFlg: false, //年度目标标识
			aimConfigForm: {
				id: null,
				name: '', //目标名称 目标代码
				respDepartment: null, //责任科室
				respPerson: null, //责任人
				yearmonthType: '', //目标类型：year/month
				requirement: '', //目标执行说明
	...
	created() {
		this.checkYearAim() //判断是否是年度目标
		const respPerName = this.respPerNameQuery
		this.remoteMethod(respPerName)
		...
		this.getUserInfo()
		//获取用户可操作的目标参数
		this.getUsersAimParam()
	},
	mounted() {
		const data = this.aimConfigData
		if (data.id !== null && data.id !== '') {
				//修改parent bnsDataItem objectiveDataItem
				data.parent = data.parent == 0 ? null : data.parent
				data.bnsDataItem = data.bnsDataItem == 0 ? null : data.bnsDataItem
				data.objectiveDataItem = data.objectiveDataItem == 0 ? null : data.objectiveDataItem
				data.objectiveDataFormula = data.objectiveDataFormula == 0 ? null : data.objectiveDataFormula
				this.aimConfigForm = Object.assign({}, data)
		}
		if (this.dialogStatus == '发起年度目标') {
			this.aimConfigForm.yearmonthType = 'year'
		}
	},
	methods: {
		/* 判断是否为年度目标 */
		checkYearAim() {
			const yearmonthType = this.aimConfigData.yearmonthType;
			this.yearAimFlg = yearmonthType == 'year' ? true : false;
		},
		/* 获取用户id */
		getUserInfo() {
		...
		getUsersAimParam() {
			const query = {
				userID: this.aimConfigForm.operator
			}
			getAimConfigParamByUser(query).then(res => {
				this.usersAimParams = res.data
			})
		},
		/* 获取父级目标的权重分配明细 */
		...
		resetForm() {	
			...
			this.aimConfigForm.parent = null //用户可用目标参数
