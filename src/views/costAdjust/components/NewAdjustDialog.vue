<template>
	<div id="costAdjustDialog" style="height: 650px; width: 1500px; ">
		<el-dialog 
			...
			&nbsp;
			<label class="filter-container-right-label">调整总金额</label><label class="filter-container-right-label-num">{{sumAdd}}</label>
        ...
        height="450" useVirtual>
				...
				<el-table-column label="成本项目">					
					<template slot-scope="scope">
						<el-select 
							v-if="scope.row.edit"
							v-model="scope.row.costTypeId" 
							filterable 
							placeholder="请选择成本项目">
							<el-option
			   ...
			   <el-table-column label="调整科室">
			    <template slot-scope="scope">
			      <el-select 
							v-if="scope.row.edit"
			      	v-model="scope.row.deptmentId" 
							filterable 
			      	placeholder="请选择调整科室">
			      	<el-option 
			  ...
			  <el-table-column label="调整金额">
			    <template slot-scope="scope">
			      <el-input type = 'number' v-if="scope.row.edit"
						v-model.trim="scope.row.adgustValue"
			  ...
			  <el-table-column label="调整原因">
			    <template slot-scope="scope">
			     <el-input type = 'text' v-if="scope.row.edit"
						v-model.trim="scope.row.adgustReason" 
						style="width: 280px;">
					</el-input>
...
import { saveCostTypeData } from "@/api/costAdjust";
  ...
  watch: {
    assigneeList: {
      handler(val, oldVal) {
        ...
        //   val[this.selectedIndex].adgustValue = parseFloat(
        //     val[this.selectedIndex].adgustValue
        //   );
          // val[this.selectedIndex].modifyValue2 = 0- parseFloat(val[this.selectedIndex].originalValue2)  *  parseFloat(val[this.selectedIndex].ratio2)/100.00
          this.sumIncomeData();
        }
      },
      deep: true
  ...
  methods: {
    ...
    sumIncomeData() {
      ...
      this.assigneeList.map((data, index) => {
        // console.log(data);
        if (isNaN(data.adgustValue)) {
          self.sumAdd = self.sumAdd + 0.0;
        } else {
          self.sumAdd = self.sumAdd + parseFloat(data.adgustValue);
        }
      });
    },
    valideAssignee(data) {
      ...
      // console.log(data);
      if (data.costTypeId === null) {
        this.canSave = false;
        self.$message({
          message: "未选择成本类别",
          type: "warning"
        });
      } else if (data.adgustValue === 0 && data.deptmentId === null) {
        this.canSave = false;
        self.$message({
          message: "未选择科室和输入调整金额",
          type: "warning"
        });
    ...
    confirmAssignee(data) {
      ...
      if (data.costTypeId === null) {
        ...
        saveCostTypeData(data).then(res => {
          self.$notify({
            title: "成功",
            message: res.msg,
    ...
    copyData(index, data) {
      ...
      let copyAssignee = {
        ...
        deptmentId: null,
        adgustValue: data.adgustValue,
        adgustReason: data.adgustReason,
        edit: true,
        subLoading: false
      };
