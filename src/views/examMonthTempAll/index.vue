...
<el-select 
  ...
  <div v-show="isShow">
    <el-table ref="listTable"  :data="tableData" v-loading="listLoading"  style="width: 100%">
      ...
      <el-table-column prop="yearStr" label="考核年度">
      </el-table-column>
      <el-table-column prop="tempName" label="考核模板">
      </el-table-column>
      <el-table-column prop="deptName" label="考核科室">
      </el-table-column>
      <el-table-column prop="score1" label="1月考核分数">
      </el-table-column>
       <el-table-column prop="score2" label="2月考核分数">
      </el-table-column>
      <el-table-column prop="score3" label="3月考核分数">
      </el-table-column>
      <el-table-column prop="score4" label="4月考核分数">
      </el-table-column>
       <el-table-column prop="score5" label="5月考核分数">
      </el-table-column>
      <el-table-column prop="score6" label="6月考核分数">
      </el-table-column>
      <el-table-column prop="score7" label="7月考核分数">
      </el-table-column>
      <el-table-column prop="score8" label="8月考核分数">
      </el-table-column>
      <el-table-column prop="score9" label="9月考核分数">
      </el-table-column>
      <el-table-column prop="score10" label="10月考核分数">
      </el-table-column>
      <el-table-column prop="score11" label="11月考核分数">
      </el-table-column>
      <el-table-column prop="score12" label="12月考核分数">
      </el-table-column>
    ...
    <el-dialog :title="tableTitle" :show-close="true" :close-on-click-modal="false" :visible.sync="tableShow" fullscreen>
      <el-card class="box-card" v-for="(item,index) in allList" :key="index">
        <el-table :data="item.unionAppliedTargets" v-loading="loading" style="width: 100%" :span-method="objectSpanMethod" border>
...
export default {
  ...
  methods: {
    setDefaultYearmonth() {
      ...
      var defYearmonth = new Date();
      const ymStr = formatDate(defYearmonth, "yyyy");
      self.dTime = ymStr;
    ...
    getList() {
      ...
      getOneYearExamTempData(self.listQuery).then(res => {
        console.log(res);
...
sourcesSave() {
    ...
    selectAble(row){
      if (row.rectificationReportState === 1 || row.rectificationReportState === 2|| row.rectificationReportState === 3 ) {
        return true
      }else{
        return false
      }
