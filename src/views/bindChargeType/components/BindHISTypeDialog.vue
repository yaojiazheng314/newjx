<template>
	<div id="assigneeDialog">
        ...
        height="450" useVirtual>
			   ...
			   <el-table-column label="HIS收费分类">
			    <template slot-scope="scope">
			      <el-select 
							v-if="scope.row.edit"
			      	v-model="scope.row.itfChargeId" 
							filterable 
			      	placeholder="请选择HIS收费分类">
			      	<el-option
...
import { fetchBindHisTypeList,fetchHisTypeList,saveBaseItfChargType,delSelectedData } from '@/api/binding'
...
export default {
	...
	methods: {
		...
		getList() {
			...
			fetchBindHisTypeList(query).then(res => {
		...
		addNewAssignee() {
			...
			let newAssignee = {
				id: null,
				baseChargeId: self.feedBackData.id,
				hospId: null,
				itfChargeId:null,
				edit: true,
				subLoading: false
		...
		confirmAssignee(data) {
			...
			}else if(data.itfChargeId === null){
				self.$message({
				  message: '请选择HIS收费分类',
				  type: 'warning'
				})
			} else {
				saveBaseItfChargType(data).then(res => {
					self.$notify({
					  title: '成功',
					  message: res.msg,
					  type: 'success',
		...
		delData(index, data) {
			...
			if (data.id === null || data.id === '') {
				...
				let assignee = {
					id: data.id,
					baseChargeId: data.baseChargeId,
					itfChargeId: data.itfChargeId
				}
				delSelectedData(assignee).then(res => {
					self.$notify({
					  title: '删除成功',
