<template>
  <div class="app-container">
		...
		<el-table
			:data="list.filter(data => !searchDR || data.deptName.toLowerCase().includes(searchDR.toLowerCase())).filter(data => !searchPR || data.perName.toLowerCase().includes(searchPR.toLowerCase()))"
      ...
      height="650" useVirtual>
			...
			<el-table-column prop="deptName" align="center" label="所属科室" />
			<el-table-column
					...
					<div v-if="scope.row.edit">
						<el-tooltip
							...
							<el-button
								...
								@click="submitItemPerValue(scope.row)"
								plain
								circle></el-button>
						</el-tooltip>
						<el-tooltip
							...
							<el-button
								...
								icon="el-icon-delete"
								size="mini"
								@click="delItemPerValue(scope.row)"
								plain
								circle></el-button>
						</el-tooltip>
					</div>
...
import { getHandleItemPerData,
				 savePerItemReport,
				 copyPerReportData,
				 delPerItemReport } from '@/api/bnsDataItemReport'
  ...
  methods: {
    ...
    fetchData() {
      var self = this
			...
			getHandleItemPerData(query).then(response => {
		...
		submitItemPerValue(data) {
			var self = this
			if (data.val === null || data.val === '') {
				self.$message({
				  message: data.perName + "提交的填报值不能为空或非数字格式",
				  type: 'error',
				  duration: 6000,
					showClose: true
				})
				...
				savePerItemReport(bnsDataItemReport).then(response => {
		...
		delItemPerValue(data) {
			...
			if (data.val === null || data.val === '') {
				...
				data.subLoading = true
				delPerItemReport(bnsDataItemReport).then(response => {
					let resultStatus = response.status
					if (resultStatus === 520) {
						data.subLoading = false
						self.$confirm(response.msg, '提示', {
		...
		copyData() {
			...
			copyPerReportData(copyParam).then(res => {
